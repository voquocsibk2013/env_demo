...
function FootprintTab({ project, onChange }) {
  ...
  const ingestFile = async (file) => {
    ...
    // Clone once and transfer the clone; wbRef keeps the original for re-calc
    // (same pattern as the CALC call below).
    const bufCopy = buf.slice(0);
    w.postMessage({ type: "DETECT", buffer: bufCopy, fileName: file.name }, [bufCopy]);
  };
