...
function ProjectView({ project, allProjects, onChange, onDelete, initialTab }) {
  ...
  const exportPDF = () => {
    try {
      ...
      let rRows="";
      [5,4,3,2,1].forEach(sv=>{let cells="";[1,2,3,4,5].forEach(pb=>{const items=mGrid[sv+","+pb]||[];const dots=items.map(a=>{const s=calcSig(a);const dc=s==="SIGNIFICANT"?"#DC2626":s==="MEDIUM"?"#D97706":"#16A34A";const op=a.status==="Info"?"0.35":"1";return'<span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:'+dc+';opacity:'+op+';margin:1px"></span>';}).join("");cells+='<td style="width:46px;height:46px;background:'+zBg(sv,pb)+';border:1px solid #e5e7eb;padding:2px;vertical-align:top;position:relative"><span style="position:absolute;top:2px;left:3px;font-size:7px;color:#9ca3af;font-weight:bold">'+(sv*pb)+'</span><div style="display:flex;flex-wrap:wrap;justify-content:center;align-items:center;margin-top:12px">'+dots+'</div></td>';});rRows+='<tr><td style="text-align:center;padding:0 6px;vertical-align:middle;width:90px"><div style="font-size:11px;font-weight:700;color:#1e293b;line-height:1.2">'+sv+'</div><div style="font-size:8px;color:#64748b;line-height:1.2">'+cLbl[sv]+'</div></td>'+cells+'</tr>';});
